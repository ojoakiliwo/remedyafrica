...
export default function PractitionerDashboard() {
      ...
      try {
        ...
        if (userDoc.exists()) {
          ...
          if (data.role === 'practitioner' || data.role === 'admin') {
            ...
            const profDoc = await getDoc(doc(db, 'practitioners', user.uid));
            if (profDoc.exists()) {
              setProfile(profDoc.data() as PractitionerProfile);
            }
          } else {
            toast.error('Access denied. Practitioner only.');
            router.push('/');
            return;
          }
        }
  ...
  // Load consultations with proper cleanup
  useEffect(() => {
    if (!user?.uid || !isPractitioner) return;
  ...
  const toggleAvailability = async () => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'practitioners', user.uid), {
        isAvailable: newStatus,
        updatedAt: serverTimestamp()
      });
      setProfile(prev => prev ? { ...prev, isAvailable: newStatus } : { isAvailable: newStatus });
          ...
          <div className="space-y-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                <Link href="/practitioners/profile/edit" className="block">
              ...
              <CardContent>
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-semibold">{profile?.totalConsultations || stats.completed}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
