...
const STORAGE_BUCKET = 'remedyafricaojo.firebasestorage.app'; // Your bucket from memory
const DELAY_MS = 1500; // Be nice to Wikimedia Commons
const MIN_WIDTH = 400; // Skip tiny images
...
async function processHerb(herbDoc) {
  ...
  if (data.images && data.images.length > 0 && data.images[0]?.url) {
    return { status: 'skipped', name, reason: 'already_has_images' };
  }
  ...
  // Make it publicly readable (optional - depends on your rules)
  await bucket.file(destination).makePublic().catch(() => {
    // If makePublic fails, the URL might still work depending on your rules
  });
